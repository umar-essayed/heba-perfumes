...
export default function ShopClient({ initialProducts }: ShopClientProps) {
  ...
  // Client-side revalidation to always have the latest prices from Firestore
  useEffect(() => {
    fetch('/api/products')
      .then(res => res.json())
      .then(data => {
  ...
  const categories = [
    { id: 'all', name: 'الكل', count: products.length },
    {
      id: 'men',
      name: 'رجالي',
      count: products.filter(p => p.gender === 'رجالي' || p.gender === 'رجال').length,
    },
    {
      id: 'women',
      name: 'حريمي',
      count: products.filter(p => p.gender === 'حريمي' || p.gender === 'نساء').length,
    },
    {
      id: 'mix',
      name: 'ميكس (الاتنين)',
      count: products.filter(p => p.gender === 'الاتنين' || p.gender === 'للجنسين').length,
    },
  ];
  ...
  return (
    <div className="min-h-screen py-10 sm:py-16 bg-[#0A0A0A]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Products Grid: 2 columns on mobile with staggered offset, 3-4 columns on desktop */}
